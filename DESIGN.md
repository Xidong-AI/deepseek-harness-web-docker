...
| 运行阶段工具链 | 运行镜像不含编译工具链（瘦身） | 运行镜像恢复轻量编译工具链（make/gcc/g++/pkg-config）+ Rust（rustup stable minimal + rustfmt + clippy） | agent 运行时装 dsh 插件（dsh-better-sidebar 等）触发 node-pty 现场编译（prebuild 未匹配时回退 node-gyp），缺 make 直接报 `not found: make`；Rust 供 agent `cargo build` 编译项目。RUSTUP_HOME/CARGO_HOME 置系统目录并 chown node，cargo/bin 软链至 /usr/local/bin（agent PATH 白名单固定）。取舍：镜像体积回升（+~600MB），换取运行时 native 模块自编译能力，避免每次插件安装都需重建镜像 |
