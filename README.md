...
- **Authentication since dsh 0.1.5-rc.2**: moved into dsh itself. On first start, dsh prints a one-time `?token=…` URL; the browser exchanges it for a persistent cookie (survives restarts). Caddy basic auth was removed in 0.1.5-rc.2+ (would have intercepted the browser's first redirect and hidden the token URL)
...
> ⚠️ **升级警告 — 升级到含 dsh 0.1.5-rc.2+ 鉴权迁移的版本时，必须先 `git pull` 同步 `docker-compose.yml` / `Caddyfile` / `entrypoint.sh` / `entrypoint.sh 内的 token 抓取后台任务`,仅 `docker compose pull` 拉新 image 配旧 compose 文件会导致 healthcheck 永远 unhealthy 且 web 服务裸奔 (无鉴权)。** 具体见 PR #7 的 commit message 与 DESIGN.md §10 的版本演进记录。
...
| --- | --- | --- | --- |
| `DSH_AUTH_USER` | No (since 0.1.5-rc.2) | `admin` | Was: Basic Auth username. Now kept only for rollback to ≤0.1.1 series; ignored on 0.1.5-rc.2+ |
| `DSH_AUTH_PASSWORD` | No (since 0.1.5-rc.2) | None | Was: Basic Auth plaintext password (bcrypt hash auto-generated). Now kept only for rollback to ≤0.1.1 series |
